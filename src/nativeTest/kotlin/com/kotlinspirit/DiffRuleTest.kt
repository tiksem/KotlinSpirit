...
class DiffRuleTest {
    private fun <T : Any> verifyDiffFailed(rule: Rule<T>, str: String, code: Int = ParseCode.DIFF_FAILED) {
        ...
        assertEquals(result.parseResult.getParseCode(), code)
        assertEquals(result.endSeek, 0)
    }

    private fun <T : Any> verifyDiffSuccess(rule: Rule<T>, str: String) {
        assertEquals(rule.compile().parseOrThrow(str), str.length)
    }
    ...
    fun noTestWordButNot() {
        ...
        val r = !(word - "Hello")
        assertEquals(r.compile().tryParse("Hi Yo Hello"), null)
        assertEquals(r.compile().tryParse("Hello world!"), 1)
        assertEquals(r.compile().tryParse("Hello World!"), 1)
        assertEquals(r.compile().tryParse("Hi"), null)
        assertEquals(r.compile().tryParse("345634Hello"), 1)
        assertEquals(r.compile().tryParse(" Hello 3445 Hi"), 1)
    }
    ...
    fun isNotComment() {
        ...
        assertEquals(p.tryParse("12345"), "12345".length)
        assertEquals(p.tryParse("12345/*"), "12345/*".length)
        assertEquals(p.tryParse("/*"), "/*".length)
        assertEquals(p.tryParse("/*edssdds*"), "/*edssdds*".length)
        assertEquals(p.tryParse("abcd/*edssdds*/**/"), "abcd".length)
        assertEquals(p.tryParse("abcd/edssdds**4/**/"), "abcd*edssdds**4".length)
        assertEquals(p.tryParse("/*3434*/"), null)
