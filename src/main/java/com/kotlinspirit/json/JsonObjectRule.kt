...
class JsonObjectRule(name: String? = null) : RuleWithDefaultRepeat<ParseRange>(name) {
    override fun clone(): JsonObjectRule {
        return this
    }

    override fun parse(seek: Int, string: CharSequence): ParseSeekResult {
        val parser = JsonParser()
        val endSeek = parser.parseJsonObject(seek, string)
            ?: return ParseSeekResult(seek, ParseCode.INVALID_JSON_OBJECT)
    ...
    override fun name(name: String): JsonObjectRule {
        return JsonObjectRule(name)
    }
