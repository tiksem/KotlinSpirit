...
abstract class BaseReplace {
    ...
    internal fun appendListReplacements(
        ...
        for (l in listReplacements) {
            for (i in 0 until l.ranges.size) {
                replacements.add(l.replacementProvider(string, i))
            }

            for (range in l.ranges) {
...
class Replacer(
    ...
    private val replace = createThreadLocal<Replace> {
        provider().let {
            if (debug) {
                val engine = debugEngine.get()
                it.debug(engine)
            } else {
                it
            }
    ...
    private val debugEngine = createThreadLocal { DebugEngine() }

    private fun get(string: CharSequence): Replace {
        return replace.get().apply {
