...
private val no = (!Rules.bigDecimal).compile()
...
class BigDecimalTest {
    ...
    fun testInteger() {
        ...
        assertEquals(
            Rules.bigDecimal.parse(0, str), createStepResult(
                seek = str.length,
    ...
    private fun testResult(string: String) {
        assertEquals(
            Rules.bigDecimal.compile().parseGetResultOrThrow(string),
            BigDecimal(string)
        )
    }
    ...
    fun largeExponent() {
        ...
        assertEquals(ParseCode.BIG_DECIMAL_EXPONENT_OVERFLOW, Rules.bigDecimal.compile().parse(str).errorCode)
    }

    @Test
    fun testSpaceAfterDouble() {
        val result = ParseResult<BigDecimal>()
        Rules.bigDecimal.parseWithResult(0 , "5.0  ", result)
        assertEquals(result.data ?: BigDecimal(-1.0), BigDecimal("5.0"))
    ...
    @Test
    fun noParseTest() {
        assertEquals(no.tryParse("......."), 1)
    }

    @Test
    fun noParseTest3() {
        assertEquals(no.tryParse("abcdegfrt9.0"), 1)
    }

    @Test
    fun noParseTest4() {
        assertEquals(no.tryParse("+__fff.0"), 1)
    }

    @Test
    fun noParseTest5() {
        assertEquals(no.tryParse("-__fff"), 1)
    }

    @Test
    fun noParseTest6() {
        assertEquals(no.tryParse("+.er4"), 1)
    }
